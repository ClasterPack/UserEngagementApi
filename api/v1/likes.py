...
async def create_like(like: Like, current_user: dict = Depends(get_current_user)):
    logger.info("запрос на создание like")
    if str(like.user_uid) != str(current_user["user_uid"]):
        raise HTTPException(
            status_code=403, detail="Нельзя ставить лайк от имени другого пользователя"
        )
...
async def list_likes(current_user: dict = Depends(get_current_user)):
    logger.info(
        f"Пользователь {current_user['user_uid']} запрашивает список своих лайков"
    )
...
async def get_like(like_id: UUID, current_user: dict = Depends(get_current_user)):
...
async def update_like(
    like_id: UUID, like: Like, current_user: dict = Depends(get_current_user)
):
    ...
    result = await update_item(
        collection_name, like_id, like.model_dump(exclude_unset=True)
    )
    return result


@router.delete("/{like_id}")
async def delete_like(like_id: UUID, current_user: dict = Depends(get_current_user)):
