...
async def create_review(review: Review, current_user: dict = Depends(get_current_user)):
    ...
    if str(review.user_uid) != str(current_user["user_uid"]):
        raise HTTPException(
            status_code=403,
            detail="Нельзя создавать отзыв от имени другого пользователя",
        )
...
async def list_reviews(current_user: dict = Depends(get_current_user)):
    logger.info(
        f"Пользователь {current_user['user_uid']} получает список своих отзывов"
    )
...
async def get_review(review_id: UUID, current_user: dict = Depends(get_current_user)):
    logger.info(
        f"Пользователь {current_user['user_uid']} запрашивает отзыв {review_id}"
    )
...
async def update_review(
    review_id: UUID, review: Review, current_user: dict = Depends(get_current_user)
):
    logger.info(
        f"Пользователь {current_user['user_uid']} редактирует отзыв {review_id}"
    )
    ...
    result = await update_item(
        collection_name, review_id, review.model_dump(exclude_unset=True)
    )
    return result
...
async def delete_review(
    review_id: UUID, current_user: dict = Depends(get_current_user)
):
